...
# Data directories (warn on failure, don't abort)
...
restore_from_backup() {
  ...
  local backup_url="gs://${bucket}/backups/${backup_name}"
  local restore_file="/tmp/openclaw-restore.tar.gz"
  ...
  # Restore data directories (warn on failure)
  for dir in credentials identity agents memory extensions devices cron canvas completions media subagents; do
  ...
  # Restore browser data + strip caches
